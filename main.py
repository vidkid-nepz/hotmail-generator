...
class HotmailAccountCreator:
    ...
    async def human_click(self, element):
        if not self.page:
            return
        """Enhanced human-like clicking with background automation support"""
        
        try:
            # CRITICAL: Ensure window focus before clicking (for background automation)
            try:
                await self.page.bring_to_front()
                await self.page.evaluate("window.focus()")
            except:
                pass  # Continue if focus fails
            
            # Get element position
            box = await element.bounding_box()
            if not box:
                # Could not get element bounding box, using JavaScript click
                # Direct JavaScript click if no bounding box
                await element.evaluate("el => el.click()")
                await asyncio.sleep(random.uniform(0.2, 0.5))
                return True
            
            # Calculate realistic click position (not always center)
            x_variance = random.uniform(-0.3, 0.3)  # ±30% from center
            y_variance = random.uniform(-0.2, 0.2)  # ±20% from center
            
            click_x = box['x'] + (box['width'] * (0.5 + x_variance))
            click_y = box['y'] + (box['height'] * (0.5 + y_variance))
            ...
            # For background automation: Use JavaScript click primarily, mouse click as secondary
            try:
                # Method 1: JavaScript click (works better in background)
                await element.evaluate("el => el.click()")
                # JavaScript click completed
                await asyncio.sleep(random.uniform(0.2, 0.5))
                return True
            except Exception as e1:
                # JavaScript click failed, trying mouse click
                try:
                    # Method 2: Mouse click (as backup)
                    await self.page.mouse.click(click_x, click_y, delay=random.randint(50, 150))
                    # Mouse click completed
                    await asyncio.sleep(random.uniform(0.2, 0.5))
                    return True
                except Exception as e2:
                    # Mouse click failed, trying element click
                    # Method 3: Element click (final fallback)
    ...
    async def human_mouse_move_to(self, target_x, target_y):
        if not self.page:
            return
        """Move mouse to target position in a human-like curved path"""
        try:
            # Get current mouse position
            current_mouse = await self.page.evaluate("""
                () => {
                    if (!window.lastMouseX) window.lastMouseX = Math.random() * 500 + 100;
                    if (!window.lastMouseY) window.lastMouseY = Math.random() * 300 + 100;
            ...
            steps = max(5, min(int(distance / 10), 20))
            
            # Create curved path
            for i in range(steps + 1):
                t = i / steps
                
                # Add curve to make movement more natural
                curve_factor = random.uniform(-0.2, 0.2) * distance * (1 - abs(t - 0.5) * 2)
                
                # Calculate intermediate position
                x = start_x + (target_x - start_x) * t + curve_factor
                y = start_y + (target_y - start_y) * t + curve_factor * 0.5
                ...
                if t < 0.2 or t > 0.8:
                    delay = random.uniform(0.02, 0.05)  # Slower at start/end
                else:
                    delay = random.uniform(0.01, 0.03)  # Faster in middle
    ...
    async def human_type(self, element, text: str):
        ...
        try:
            # First, clear the field naturally
            await element.click()
            await asyncio.sleep(random.uniform(0.1, 0.3))
            
            # Select all and delete (like humans do)
            ...
            for word_index, word in enumerate(words):
                if word_index > 0:
                    # Type the @ symbol with a slight pause
                    await element.type('@', delay=random.randint(120, 280))
                    await asyncio.sleep(random.uniform(0.1, 0.3))
                ...
                for i, char in enumerate(word):
                    ...
                    if char.isalpha():
                        delay = random.randint(80, 180)  # Letters
                    elif char.isdigit():
                        delay = random.randint(100, 220)  # Numbers (slightly slower)
                    elif char in '._-':
                        delay = random.randint(150, 300)  # Special chars (slower)
                    else:
                        delay = random.randint(90, 200)  # Other chars
                    
                    # Add variance for realistic typing
                    if i % 3 == 0:  # Every 3rd character slightly slower
    ...
    async def handle_password_page(self, password):
        if not self.page:
            return
        try:
            ...
            password_field_selectors = [
                'input#floatingLabelInput13',
                'input[type="password"]',
                'input[autocomplete="new-password"]',
                'input[aria-describedby="field-14__validationMessage"]',
                'input.fui-Input__input',
                'input[class*="fui-Input__input"]',
                'input[class*="r12stul0"]',
                'input[class*="___1b2swfv"]',
                'input[class*="fk8j09s"]',
                'input[class*="fdw0yi8"]',
            ]
            
            password_field = await self.find_first_selector(password_field_selectors, timeout=3000)
            ...
            next_button = await self.find_first_selector(next_button_selectors, timeout=3000)
            if not next_button:
                return False
